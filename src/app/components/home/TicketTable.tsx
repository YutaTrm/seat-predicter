    ...
    <div className="flex flex-col">
        ...
        <table className="min-w-full bg-white border relative">
          <thead className="sticky top-0 bg-gray-600">
            <tr className="text-gray-100 text-xs">
              ...
              >
                {t('table.block')}{renderSortIcon('block')}
              </th>
              <th
                className="px-2 py-1 border-b text-center font-semibold cursor-pointer hover:bg-gray-500"
                onClick={() => toggleSort('column')}
              >
                {t('table.row')}{renderSortIcon('column')}
              </th>
              <th
                className="px-2 py-1 border-b text-center font-semibold cursor-pointer hover:bg-gray-500"
                onClick={() => toggleSort('number')}
              >
                {t('table.seat')}{renderSortIcon('number')}
              </th>
              <th
                className="px-2 py-1 border-b text-center font-semibold cursor-pointer hover:bg-gray-500"
                onClick={() => toggleSort('lottery_slots_name')}
              >
                {t('table.lotterySlot')}{renderSortIcon('lottery_slots_name')}
              </th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className="flex gap-8">
        <div className="flex-1">
          <h3 className="text-gray-600 font-bold mb-2">{t('table.blockSummary')}</h3>
          <table className="">
            <tbody>
              {Object.entries(blockCounts).map(([block, count]) => (
                <tr key={block} className="text-sm p-2 rounded">
                  ...
                  <td className="text-right w-[4em]">
                    <span className='text-rose-500'>{count}</span>{t('table.count')}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="flex-1">
          <h3 className="text-gray-600 font-bold mb-2">{t('table.lotterySlotSummary')}</h3>
          <table className="">
            <tbody>
