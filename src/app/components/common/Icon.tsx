/**
 * アイコンを表示するコンポーネント
 * @param type - アイコンの種類（'edit': 編集ペン, 'delete': ゴミ箱, 'menu': 縦三点リーダー, 'rotate': 回転矢印）
 */
type IconProps = {
  type: 'edit' | 'delete' | 'menu' | 'rotate'
}

export default function Icon({ type }: IconProps) {
  // typeに応じてパス定義を返す
  const getPathDefinition = (type: 'edit' | 'delete' | 'menu' | 'rotate'): string => {
    switch (type) {
      case 'edit':
      ...
      case 'menu':
        return "M12 6.75a.75.75 0 110-1.5.75.75 0 010 1.5zM12 12.75a.75.75 0 110-1.5.75.75 0 010 1.5zM12 18.75a.75.75 0 110-1.5.75.75 0 010 1.5z"
      case 'rotate':
